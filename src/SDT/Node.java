...
class Node {
        ...
        for (int e = 0; e < MAX_EPOCH; e++) {
            ...
            for (int i = 0; i < X.size(); i++) {
                ...
                for (int count = 0; count < ATTRIBUTE_COUNT; count++)
                    dw[count] = (-t * (leftNode.w0 - rightNode.w0) * g * (1 - g)) * x[count];

                dw0 = (-t * (leftNode.w0 - rightNode.w0) * g * (1 - g));
                dw10 = -t * (g);
                dw20 = -t * (1 - g);
    ...
    void splitNode(final ArrayList<Instance> X, final ArrayList<Instance> V,final SDT tree) {
...
//            rightNode.splitNode(X, V, tree);
            ...
            SDTMain.split_q.add(leftNode);
            SDTMain.split_q.add(rightNode);
