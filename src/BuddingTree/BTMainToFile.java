            ...
            public void run() {
                try {
                    ...
                    for (String s : CLASSIFY) {
                        ...
                        for (int i = 1; i <= 5; i++) {
                            for (int j = 1; j <= 2; j++) {
                                BT sdt = new BT("data_sdt\\" + s + "\\" + s + "-train-" + i + "-" + j + ".txt", "data_sdt\\" + s + "\\" + s + "-validation-" + i + "-" + j + ".txt", "data_sdt\\" + s + "\\" + s + "-test.txt", false, LEARNING_RATE, EPOCH, MAX_STEP);
                                sdt.learnTree();
